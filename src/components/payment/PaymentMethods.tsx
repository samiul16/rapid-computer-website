...
const PaymentMethods: React.FC<PaymentMethodsProps> = ({
  ...
  return (
    <div
      className={`min-w-64 w-full p-6 bg-[#20B8FB0D] rounded-xl outline-1 outline-black/20 flex flex-col gap-6 ${className}`}
    ...
    >
      ...
      <div className="flex flex-col gap-4">
        {methods.map((method) => (
          <label
            key={method}
            className="flex items-center gap-3 cursor-pointer group hover:bg-black/5 p-2 rounded-lg transition-all duration-200"
          >
      ...
      {localSelectedMethod && (
        <div className="mt-2 p-3 bg-[#26ADDF]/10 rounded-lg border-l-4 border-[#26ADDF] animate-slide-in">
